...
function fn_search(page){
    ...
    $.ajax({
        url: '${ctxt}/execute/evaluate/readProjEvalList.do',
        data: params,
        type: 'POST',
        dataType: 'json',
        success: function(result) {
        	...
        	pagetotalCnt = 0;
        	if(!isEmpty(result)){
				if(!isEmpty(result.evalList)){
					var start_num = Number(result.evalTotal) - ((cuurPage -1) *10)
					pagetotalCnt =Number(result.evalPageTotal);	
					
					$.each(result.evalList, function(idx, item){
						html += '<tr>';
							html += '<td>' + (start_num - idx) + '</td>';
							html += '<td>'+((isEmpty(item.dept_org)) ? '' : item.dept_org) +'</td>';
							html += '<td>'+((isEmpty(item.ddct_org)) ? '' : item.ddct_org) +'</td>';
							html += '<td class="text_l"><a href="javascript:fn_evalDtl(\''+item.proj_year_id+'\');">'+((isEmpty(item.proj_nm_kor)) ? '' : item.proj_nm_kor) +'</a></td>';
...
// 연차과제 상세 상세 
function fn_evalDtl(yearSeq){
	
	$('#proj_year_id').val(yearSeq);
	var form = document.writeForm;
	form.action = '${ctxt}/execute/evaluate/projEvalDetail.do';
	form.submit();
...
<div id="container">
	<div id="divRefreshArea">
		<form name="writeForm" method="post" action="">
			<h3 class="page_title" id="title_div"><span class="adminIcon"></span>과제평가 관리</h3>  
			...
			<div class="grid_box">
				...
				<!-- 연차과제명 검색 -->
				연차과제명 : <input type="text" name="searchword" id="searchword"  value="" style="" onkeypress="javascript:if(event.keyCode == 13) { fn_search(1); return false;}" />
				
				<a href="javascript:void(0);" id="btnSearch" class="btn btn-primary" onclick="fn_search(1)" >검색</a>
			 	<input type="hidden" name="pageSize" value="10"/>
				<input type="hidden"  id="page" name="page" value="${evalParam.page}" />				
		...
		<div class="admin ma_t_20"> 
			<table class="table_h" cellpadding="0" cellspacing="0" border="0" id="admTable"> 
				<caption>
				 	  평가관리 -  번호, 소관기관 , 전담기관, 연차과제명, 단계구분, 연차구분, 수행기간
				</caption>
				<colgroup>
					<col style="width:5%">	
					<col style="width:12%">
