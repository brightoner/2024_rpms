...
function fn_search(page){
    ...
    $.ajax({
        url: '${ctxt}/follow/year/planInfo/readYearFollowList.do',
        data: params,
        type: 'POST',
        dataType: 'json',
				...
				if(!isEmpty(result.planList)){
					...
					$.each(result.planList, function(idx, item){
						html += '<tr>';
							...
							html += '<td class="text_l"><a href="javascript:fn_yearFollowDtl(\''+item.proj_year_id+'\');">'+((isEmpty(item.proj_nm_kor)) ? '' : item.proj_nm_kor) +'</a></td>';
...
// 연차과제 상세 상세 
function fn_yearFollowDtl(yearSeq){
	
	$('#proj_year_id').val(yearSeq);
	var form = document.writeForm;
	form.action = '${ctxt}/follow/year/planInfo/yearFollowDetail.do';
	form.submit();
...
<div id="container">
	<div id="divRefreshArea">
		<form name="writeForm" method="post" action="">
			<h3 class="page_title" id="title_div"><span class="adminIcon"></span>연차과제 수행 관리</h3>  
			...
			<div class="grid_box">
				...
				<!-- 신청과제명 검색 -->
				연차과제명 : <input type="text" name="searchword" id="searchword"  value="${followParam.searchword}" style="" onkeypress="javascript:if(event.keyCode == 13) { fn_search(1); return false;}" />
		...
		<div class="admin ma_t_20"> 
			<table class="table_h" cellpadding="0" cellspacing="0" border="0" id="admTable"> 
				<caption>
				 	  사후관리 -  번호, 소관기관 , 전담기관, 연차과제명, 단계구분, 연차구분, 수행기간
				</caption>
				<colgroup>
					<col style="width:5%">	
					<col style="width:12%">
