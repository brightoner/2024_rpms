...
// 		}
	...
	//첨부파일등록 팝업 호출
	function fn_egov_file_popup7() {
		var reqManageVo=document.employFileForm;
		var file_group = reqManageVo.year_file_group_employ.value;
		var file_gb = "7";
	...
	function fn_employSearch(employPage){
		...
		cuurPage= employPage;
		var proj_year_id_employ = $("#proj_year_id_employ").val();

		var params = {};
			params.page    = cuurPage;   
			params.proj_year_id = proj_year_id_employ;
			
	  	$('#employPage').val(cuurPage);
		  	
	$.ajax({
	    url: '${ctxt}/follow/year/employ/readYearEmployList.do',
	    data: params,
	    type: 'POST',
	    dataType: 'json',
				...
				if(!isEmpty(result.employList)){
					...
					$.each(result.employList, function(idx, item){
						html += '<tr name="listTr">';
							...
							html += '<td class="text_c">'+'<input type="checkbox" name="chkObj" title="선택하기'+idx+'" value='+item.year_employ_id+'>';		
							html += '<td class="text_c"><a href="javascript:fn_employDtl(\''+item.year_employ_id+'\');">'+((isEmpty(item.resp_nm)) ? '' : item.resp_nm) +'</a></td>';
							html += '<td class="text_c">'+((isEmpty(item.year_employ_dt)) ? '' : item.year_employ_dt)+'</td>';
							html += '<td class="text_c">'+((isEmpty(item.year_employ_gb)) ? '' : item.year_employ_gb)+'</td>';												
							html += '<td class="text_c">'+((isEmpty(item.year_employ_youth_gb)) ? '' : item.year_employ_youth_gb)+'</td>';												
						html += '</tr>';						
					});
	...
	function fn_employDtl(seq){
		...
		var year_employ_id = seq;
		
		var params = {};
		params.year_employ_id = year_employ_id;
		
		
		$.ajax({
	        url: '${ctxt}/follow/year/employ/yearEmployDetail.do',
	        data: params,
	        type: 'POST', 
	        dataType: 'json',	 
			success: function(result){		
				
				$("#year_employ_cd").val(result.data.year_employ_cd);
				$("#resp_nm_employ").val(result.data.resp_nm);
				$("#year_employ_dt").val(result.data.year_employ_dt);
				$("#year_employ_info").val(result.data.year_employ_info);
				$('[name=year_employ_gb]').val(result.data.year_employ_gb);
				$('[name=year_employ_youth_gb]').val(result.data.year_employ_youth_gb);   
	...
	function fn_saveEmploy(){
		...
		// 채용일자 validation
		if($("#year_employ_dt").val() == "" || $("#year_employ_dt").val() == null){
			fn_showCustomAlert("채용일을 입력 하세요.");
			$("#year_employ_dt").val("");
			$('#year_employ_dt').focus();
			return false;
		}
		
		// 고용구분 validation
		if($('[name=year_employ_gb] :selected').val() == "" || $('[name=year_employ_gb] :selected').val() == null){
			fn_showCustomAlert("고용구분을 선택 하세요.");
			$("#year_employ_gb").val("");
			$('#year_employ_gb').focus();
			return false;
		}
		
		// 청년채용구분 validation
		if($('[name=year_employ_youth_gb] :selected').val() == "" || $('[name=year_employ_youth_gb] :selected').val() == null){
			fn_showCustomAlert("청년채용구분 선택 하세요.");
			$("#year_employ_youth_gb").val("");
			$('#year_employ_youth_gb').focus();
			return false;
		}
		...
		var requestData = {
        		mainItem : item,
        		proj_year_id_employ : proj_year_id_employ
            };
		
		
	    $.ajax({
		        url: '${ctxt}/follow/year/employ/insertYearEmploy.do',
	...
	function fn_clearEmploy(){
		$("#employFrm").find('input[type="text"]').val(''); 
		$("#employFrm").find('input[type="date"]').val(''); 
		$("#year_employ_cd").val(''); 
		$("#employFrm").find('textarea').val(''); 
		$("#employFrm").find('select').val('');
	}
	...
	function fn_delEmploy(){
	    ...
	    $('input[name="year_employ_ids"]').val('');
        $('input[name="year_employ_ids"]').val(selectVal);
		...
		if(confirm("삭제하시겠습니까?") ==true ){
	        ...
	        $.ajax({
	            url: '${ctxt}/follow/year/employ/deleteYearEmploy.do',
	            type: 'POST',
	            data: sendData,
	            type: 'POST', 
	            dataType: 'json',
			...
			<form id="employFrm" name="employFrm" method="post"  action="">
				...
				<table id="employTable" class="table_h" cellpadding="0" cellspacing="0" border="0" >
					...
					<tbody>
						<tr>		
							<td>  
								<input type="hidden" id="year_employ_cd" name="year_employ_cd" value="">
							...
							<td>
								<input type="date" id="year_employ_dt" name="year_employ_dt" value="" max="9999-12-31" min="1111-01-01">	
							</td>
							<td>
								<div class="custom-select selectRow">
									<select id="year_employ_gb" name="year_employ_gb" class="select">
							...
							<td>
								<div class="custom-select selectRow">
									<select id="year_employ_youth_gb" name="year_employ_youth_gb" class="select">
									    <option value="">선택하세요.</option>
							...
							<td>
								<textarea id="year_employ_info" name="year_employ_info" rows="3" style="width:100%;ime-mode:active;"></textarea>
							</td>
						</tr>
					</tbody>
		...
		<div class="attch">
			<table class="table_v">  
				...
				<tbody>
					...
					<th>첨부파일</th>
				  	<td colspan="3">
						 <a href="javascript:void(0);" onclick="javascript:fn_egov_file_popup7();" class="btn btn-secondary">파일 올리기</a>
						 <div id="fileContainer7" class="file-container"></div>
				    </td>
				</tr>
				</tbody>
			</table>
...
<form name ="idsForm" id="idsForm" method="post" action="">
	<input type="hidden" id="year_employ_ids" name="year_employ_ids"/>
</form>

<form name ="employFileForm" id="employFileForm" method="post" action="">
	<input type="hidden" id = "year_file_group_employ" name="year_file_group_employ" value="${data.year_file_group}" />
</form>

<input type="hidden" id = "employPage" name="employPage" value="" />
<input type="hidden" id = "proj_year_id_employ" name="proj_year_id_employ" value="${data.proj_year_id}" />
