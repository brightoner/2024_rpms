...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt"%>
	...
	function fn_mngSearch(mngPage){
		...
		var proj_end_id_sales = $("#proj_end_id_sales").val();

		var params = {};
			params.page    = cuurPage;   
			params.searchopt    = $('[name=searchopt] :selected').val();   
			params.searchword    = $('#searchword').val()
			params.proj_end_id_sales = proj_end_id_sales;
			
	  	$('#mngPage').val(cuurPage);
		  	
		$.ajax({
		    url: '${ctxt}/follow/end/sales/readSalesMngList.do',
		    data: params,
		    type: 'POST',
		    dataType: 'json',
	...
	function fn_mngSave(){
		 ...
		 var mngVal = [];
			...
			if(confirm("저장하시겠습니까?") ==true ){
		        ...
		        $.ajax({
		            url: '${ctxt}/follow/end/sales/insertEndSales.do',
		            type: 'POST',
		            data: sendData,
		            type: 'POST', 
		            dataType: 'json',
	...
	function fn_salesSearch(salesPage){
		...
		$.ajax({
		    url: '${ctxt}/follow/end/sales/readEndSalesgList.do',
		    data: params,
		    type: 'POST',
		    dataType: 'json',
		    success: function(result) {
					...
					if(!isEmpty(result.salesList)){
						...
						$.each(result.salesList, function(idx, item){
							html += '<tr name="saleslistTr">';
								...
								html += '<input type="checkbox" name="salesChkObj" title="선택하기'+idx+'" value="'+item.end_sales_id+'">';
								html += '<input type = "hidden"  name = "stauts" value = "N" >';
								html += '<input type = "hidden"  name = "end_sales_id" value = "'+item.end_sales_id+'" >';
								html += '</td>';
								html += '<td class="text_c">'+((isEmpty(item.sales_item_nm)) ? '' : item.sales_item_nm)+'</td>';
								html += '<td class="text_c">'+((isEmpty(item.sales_revenue)) ? '' : addComma(Number(item.sales_revenue)))+'</td>';
								html += '<td class="text_c">'+((isEmpty(item.sales_client_nm)) ? '' : item.sales_client_nm)+'</td>';												
	...
	function fn_salesSave(){
        ...
        $('tr[name="saleslistTr"]').each(function() {
        	 if ($(this).find('input[name="stauts"]').val() === 'Y') {
        		...
        		var end_sales_id =  $(this).closest('tr').find('input[name="end_sales_id"]').val();
	            var sales_contribute = $(this).closest('tr').find('input[name="sales_contribute"]').val();

	            var item = {
	            		end_sales_id: end_sales_id,
		...
		if(confirm("저장하시겠습니까?") ==true ){
		    ...
		    $.ajax({
		        url: '${ctxt}/follow/end/sales/updateEndSales.do',
	...
	function fn_salesDel(){
	    ...
	    $('input[name="end_sales_ids"]').val('');
        $('input[name="end_sales_ids"]').val(salesVal);
		...
		if(confirm("삭제하시겠습니까?") ==true ){
	        ...
	        $.ajax({
	            url: '${ctxt}/follow/end/sales/deleteEndSales.do',
	            type: 'POST',
	            data: sendData,
	            type: 'POST', 
	            dataType: 'json',
	...
	<div id="divRefreshArea">
		...
		<div class="searchAreaFlex" style="margin-top: 15px;">  
			<div class="grid_box" style="grid-template-columns:100px 200px 70px;">
			 ...
			 <div class="btn_wrap" >			
			  		<a href="javascript:void(0);" onclick="javascript:fn_mngSave();" class="btn btn-secondary" style="width: 100px;">매출성과 저장</a>
...
<form name ="mngForm" id="mngForm" method="post" action="">
	...
	<input type="hidden" id = "proj_end_id_mng" name="proj_end_id_mng" value="${data.proj_end_id}" />
</form>
...
<input type="hidden" id = "proj_end_id_sales" name="proj_end_id_sales" value="${data.proj_end_id}" />

<form name ="salesForm" id="salesForm" method="post" action="">
	<input type="hidden" id="end_sales_ids" name="end_sales_ids"/>
</form>
