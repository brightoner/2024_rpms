...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt"%>
	...
	$(function(){
		...
		// 첨부파일 로드
		getFileList($("#end_file_group_ip").val(), '13');
	...
	//첨부파일등록 팝업 호출
	function fn_egov_file_popup13() {
		var reqManageVo=document.ipFileForm;
		var file_group = reqManageVo.end_file_group_ip.value;
		var file_gb = "13";
	...
	function fn_ipSearch(ipPage){
		...
		cuurPage= ipPage;
		var proj_end_id_ip = $("#proj_end_id_ip").val();

		var params = {};
			params.page    = cuurPage;   
			params.proj_end_id = proj_end_id_ip;
			
	  	$('#ipPage').val(cuurPage);
		  	
	$.ajax({
	    url: '${ctxt}/follow/end/ip/readEndIpList.do',
	    data: params,
	    type: 'POST',
	    dataType: 'json',
				...
				if(!isEmpty(result.ipList)){
					...
					$.each(result.ipList, function(idx, item){
						html += '<tr name="listTr">';
							...
							html += '<td class="text_c">'+'<input type="checkbox" name="ipChkObj" title="선택하기'+idx+'" value='+item.end_ip_id+'>';		
							html += '<td class="text_c"><a href="javascript:fn_ipDtl(\''+item.end_ip_id+'\');">'+((isEmpty(item.end_ip_nm)) ? '' : item.end_ip_nm) +'</a></td>';
							html += '<td class="text_c">'+((isEmpty(item.end_ip_dt)) ? '' : item.end_ip_dt)+'</td>';
							html += '<td class="text_c">'+((isEmpty(item.end_ip_no)) ? '' : item.end_ip_no)+'</td>';
							html += '<td class="text_c">'+((isEmpty(item.end_ip_gb)) ? '' : item.end_ip_gb)+'</td>';												
							html += '<td class="text_c">'+((isEmpty(item.end_ip_domestic_gb)) ? '' : item.end_ip_domestic_gb)+'</td>';												
						html += '</tr>';						
					});
	...
	function fn_ipDtl(seq){
		...
		var params = {};
		params.end_ip_id = end_ip_id;
		
		
		$.ajax({
	        url: '${ctxt}/follow/end/ip/endIpDetail.do',
	        data: params,
	        type: 'POST', 
	        dataType: 'json',	 
			success: function(result){		
				
				$("#end_ip_id").val(result.data.end_ip_id);
				$("#end_ip_no").val(result.data.end_ip_no);
				$("#end_ip_nm").val(result.data.end_ip_nm);
				$("#end_ip_dt").val(result.data.end_ip_dt);
				$('select[name=end_ip_gb]').val(result.data.end_ip_gb);
				$('select[name=end_ip_domestic_gb]').val(result.data.end_ip_domestic_gb);   
	...
	function fn_saveIp(){
		...
		// 지적재산권 이름 validation
		if($("#end_ip_nm").val() == "" || $("#end_ip_nm").val() == null){
		...
		// 지적재산권 일자 validation
		if($("#end_ip_dt").val() == "" || $("#end_ip_dt").val() == null){
		...
		// 지적재산권 번호 validation
		if($("#end_ip_no").val() == "" || $("#end_ip_no").val() == null){
		...
		// 출원/등록구분 validation
		if($('select[name=end_ip_gb] :selected').val() == "" || $('select[name=end_ip_gb] :selected').val() == null){
		...
		// 국내외구분 validation
		if($('select[name=end_ip_domestic_gb] :selected').val() == "" || $('select[name=end_ip_domestic_gb] :selected').val() == null){
			fn_showCustomAlert("국내외구분을 선택 하세요.");
			$('#end_ip_domestic_gb').focus();
			return false;
		}
		...
		var requestData = {
        		mainItem : item,
        		proj_end_id_ip : proj_end_id_ip
            };
		
		
	    $.ajax({
		        url: '${ctxt}/follow/end/ip/insertEndIp.do',
	...
	// 초기화
	function fn_clearIp(){
		document.getElementById('ipFrm').reset();
		$("#end_ip_id").val('');
	}
	...
	function fn_delIp(){
	    ...
	    $('input[name="end_ip_ids"]').val('');
        $('input[name="end_ip_ids"]').val(ipVal);
		...
		if(confirm("삭제하시겠습니까?") ==true ){
	        ...
	        $.ajax({
	            url: '${ctxt}/follow/end/ip/deleteEndIp.do',
	            type: 'POST',
	            data: sendData,
	            type: 'POST', 
	            dataType: 'json',
			...
			<form id="ipFrm" name="ipFrm" method="post"  action="">
				...
				<table id="ipTable" class="table_h" cellpadding="0" cellspacing="0" border="0" >
					...
					<tbody>
						<tr>		
							<td>
								<input type="hidden" id="end_ip_id" name = "end_ip_id" value=""/>
								<input type="text" id="end_ip_nm" name = "end_ip_nm" value=""/>
							</td>
							<td>
								<input type="date" id="end_ip_dt" name="end_ip_dt" value="" max="9999-12-31" min="1111-01-01">	
							</td>
							<td>
								<input type="text" id="end_ip_no" name = "end_ip_no" value=""/>
							</td>
							<td>
								<div class="custom-select selectRow">
									<select id="end_ip_gb" name="end_ip_gb" class="select">
							...
							<td>
								<div class="custom-select selectRow">
									<select id="end_ip_domestic_gb" name="end_ip_domestic_gb" class="select">
									    <option value="">선택하세요.</option>
		...
		<div class="attch">
			<table class="table_v">  
				...
				<tbody>
					...
					<th>첨부파일</th>
				  	<td colspan="3">
						 <a href="javascript:void(0);" onclick="javascript:fn_egov_file_popup13();" class="btn btn-secondary">파일 올리기</a>
						 <div id="fileContainer13" class="file-container"></div>
				    </td>
				</tr>
				</tbody>
			</table>
...
<form name ="ipsForm" id="ipsForm" method="post" action="">
	<input type="hidden" id="end_ip_ids" name="end_ip_ids"/>
</form>

<form name ="ipFileForm" id="ipFileForm" method="post" action="">
	<input type="hidden" id = "end_file_group_ip" name="end_file_group_ip" value="${data.end_file_group}" />
</form>

<input type="hidden" id = "ipPage" name="ipPage" value="" />
<input type="hidden" id = "proj_end_id_ip" name="proj_end_id_ip" value="${data.proj_end_id}" />
