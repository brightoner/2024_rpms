...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt"%>
	...
	$(function(){
		...
		// 첨부파일 로드
		getFileList($("#end_file_group_etc").val(), '15');
	...
	//첨부파일등록 팝업 호출
	function fn_egov_file_popup15() {
		var reqManageVo=document.etcFileForm;
		var file_group = reqManageVo.end_file_group_etc.value;
		var file_gb = "15";
	...
	function fn_etcSearch(etcPage){
		...
		cuurPage= etcPage;
		var proj_end_id_etc = $("#proj_end_id_etc").val();

		var params = {};
			params.page    = cuurPage;   
			params.proj_end_id = proj_end_id_etc;
			
	  	$('#etcPage').val(cuurPage);
		  	
	$.ajax({
	    url: '${ctxt}/follow/end/etc/readEndEtcList.do',
	    data: params,
	    type: 'POST',
	    dataType: 'json',
				...
				if(!isEmpty(result.etcList)){
					...
					$.each(result.etcList, function(idx, item){
						html += '<tr name="listTr">';
							...
							html += '<td class="text_c">'+'<input type="checkbox" name="etcChkObj" title="선택하기'+idx+'" value='+item.end_etc_id+'>';		
							html += '<td class="text_c"><a href="javascript:fn_etcDtl(\''+item.end_etc_id+'\');">'+((isEmpty(item.end_etc_nm)) ? '' : item.end_etc_nm) +'</a></td>';
							html += '<td class="text_c">'+((isEmpty(item.end_etc_gb)) ? '' : item.end_etc_gb)+'</td>';
							html += '<td class="text_c">'+((isEmpty(item.end_etc_cn)) ? '' : item.end_etc_cn)+'</td>';
							html += '<td class="text_c">'+((isEmpty(item.end_etc_info)) ? '' : item.end_etc_info)+'</td>';												
						html += '</tr>';						
					});
	...
	function fn_etcDtl(seq){
		...
		var params = {};
		params.end_etc_id = end_etc_id;
		
		
		$.ajax({
	        url: '${ctxt}/follow/end/etc/endEtcDetail.do',
	        data: params,
	        type: 'POST', 
	        dataType: 'json',	 
			success: function(result){		
				
				$("#end_etc_id").val(result.data.end_etc_id);
				$("#end_etc_nm").val(result.data.end_etc_nm);
				$("#end_etc_cn").val(result.data.end_etc_cn);
				$("#end_etc_gb").val(result.data.end_etc_gb);
				$("#end_etc_info").val(result.data.end_etc_info);
	...
	function fn_saveEtc(){
		...
		// 성과명칭 validation
		if($("#end_etc_nm").val() == "" || $("#end_etc_nm").val() == null){
		...
		// 구분 validation
		if($("#end_etc_gb").val() == "" || $("#end_etc_gb").val() == null){
		...
		// 성과내용 validation
		if($("#end_etc_cn").val() == "" || $("#end_etc_cn").val() == null){
		...
		var requestData = {
        		mainItem : item,
        		proj_end_id_etc : proj_end_id_etc
            };
		
	    $.ajax({
		        url: '${ctxt}/follow/end/etc/insertEndEtc.do',
	...
	function fn_clearEtc(){
		...
		document.getElementById('etcFrm').reset();
		$("#end_etc_id").val('');
	}
	...
	function fn_delEtc(){
	    ...
	    $('input[name="end_etc_ids"]').val('');
        $('input[name="end_etc_ids"]').val(etcVal);
		...
		if(confirm("삭제하시겠습니까?") ==true ){
	        ...
	        $.ajax({
	            url: '${ctxt}/follow/end/etc/deleteEndEtc.do',
	            type: 'POST',
	            data: sendData,
	            type: 'POST', 
	            dataType: 'json',
			...
			<form id="etcFrm" name="etcFrm" method="post"  action="">
				...
				<table id="etcTable" class="table_h" cellpadding="0" cellspacing="0" border="0" >
					...
					<tbody>
						<tr>		
							<td>
								<input type="hidden" id="end_etc_id" name="end_etc_id" value=""/>
								<input type="text" id="end_etc_nm" name = "end_etc_nm" value=""/>
							</td>
							<td>
								<input type="text" id="end_etc_gb" name = "end_etc_gb" value=""/>
							</td>
							<td>
								<textarea id="end_etc_cn" name="end_etc_cn" rows="3" style="width:100%;ime-mode:active;"></textarea>
							</td>
							<td>
								<textarea id="end_etc_info" name="end_etc_info" rows="3" style="width:100%;ime-mode:active;"></textarea>
							</td>
						</tr>
					</tbody>
		...
		<div class="attch">
			<table class="table_v">  
				...
				<tbody>
					...
					<th>기타 첨부파일</th>
				  	<td colspan="3">
						 <a href="javascript:void(0);" onclick="javascript:fn_egov_file_popup15();" class="btn btn-secondary">파일 올리기</a>
						 <div id="fileContainer15" class="file-container"></div>
				    </td>
				</tr>
				</tbody>
			</table>
...
<form name ="etcsForm" id="etcsForm" method="post" action="">
	<input type="hidden" id="end_etc_ids" name="end_etc_ids"/>
</form>

<form name ="etcFileForm" id="etcFileForm" method="post" action="">
	<input type="hidden" id = "end_file_group_etc" name="end_file_group_etc" value="${data.end_file_group}" />
</form>

<input type="hidden" id = "etcPage" name="etcPage" value="" />
<input type="hidden" id = "proj_end_id_etc" name="proj_end_id_etc" value="${data.proj_end_id}" />
