...
<!-- 상단메뉴 -->
	...
	<div class="header-wrap">
		<h1 id="logo"><a href="/"><img src="${ctxt}/resources/images/common/logo.png" alt="로고" class="" /></a></h1>
            ...
            <!-- 상단 로그인 페이지 이동 및 정보 이동 주석 이 사이트에서는 사용안함 -->
                ...
                <c:choose>
                	<c:when test="${userVo eq null}">
                		<a href="${ctxt}/login/user/login.do" id="top_a1" title="로그인 페이지로 이동">LOGIN</a>
                	</c:when>
                	<c:otherwise>
                		<a href="${ctxt}/member/infoChI/infoMember.do" title="회원 정보로 이동" onfocus="javascript:logoutTabOn();"><c:out value="${userVo.nicknm}"/>님</a>
				    	<a class="userTooltip" href="${ctxt}/login/logout.do" id="top_a3" title="로그아웃 하기" onblur="javascript:logoutTabOff();"><span>LOGOUT</span></a>
                	</c:otherwise>
                </c:choose> 
			</li>		
