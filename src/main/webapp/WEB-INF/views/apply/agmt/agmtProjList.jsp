...
$(function(){
    ...
    for (var year = currentYear; year >= startYear; year--) {
        ...
        if("${agmtParam.searchoption3}" == year ){
...
function fn_search(page){
    ...
    $.ajax({
        url: '${ctxt}/apply/agmt/readAgmtProjList.do',
        data: params,
        type: 'POST',
        dataType: 'json',
        success: function(result) {
        	...
        	pagetotalCnt = 0;
        	if(!isEmpty(result)){
				if(!isEmpty(result.agmtList)){
					var start_num = Number(result.agmtTotal) - ((cuurPage -1) *10)
					pagetotalCnt =Number(result.agmtPageTotal);	
					
					$.each(result.agmtList, function(idx, item){
						html += '<tr>';
							html += '<td>' + (start_num - idx) + '</td>';
							html += '<td>'+((isEmpty(item.dept_org)) ? '' : item.dept_org) +'</td>';
							html += '<td>'+((isEmpty(item.ddct_org)) ? '' : item.ddct_org) +'</td>';
							html += '<td class="text_l"><a href="javascript:fn_agmtDtl(\''+item.ann_id+'\', \''+item.proj_id+'\');">'+((isEmpty(item.ann_nm)) ? '' : item.ann_nm) +'</a></td>';
							html += '<td class="text_l"><a href="javascript:fn_agmtDtl(\''+item.ann_id+'\', \''+item.proj_id+'\');">'+((isEmpty(item.proj_nm_kor)) ? '' : item.proj_nm_kor) +'</a></td>';
...
// 공고현황 상세 
function fn_agmtDtl(annSeq, projSeq){
	
	$('#ann_id').val(annSeq);
	$('#proj_id').val(projSeq);
	var form = document.writeForm;
	form.action = '${ctxt}/apply/agmt/agmtProjDetail.do';
	form.submit();
...
<div id="container">
	<div id="divRefreshArea">
		<form name="writeForm" method="post" action="">
			<h3 class="page_title" id="title_div"><span class="adminIcon"></span>과제협약 처리</h3>  
			...
			<div class="grid_box">
				...
				<div class="custom-select selectRow">
					...
					<select id="searchoption1" name="searchoption1"> 
				        ...
				        <c:forEach var="dept" items="${deptList}">
				        	<option value="${dept.org_nm}" <c:if test="${agmtParam.searchoption1 == dept.org_nm}">selected="selected"</c:if>>${dept.org_nm}</option>
				        </c:forEach>
					</select>
				</div>
				...
				<div class="custom-select selectRow">
					...
					<select id="searchoption2" name="searchoption2"> 
				        ...
				        <c:forEach var="ddct" items="${ddctList}">
				        	<option value="${ddct.org_nm}" <c:if test="${agmtParam.searchoption2 == ddct.org_nm}">selected="selected"</c:if>>${ddct.org_nm}</option>
				        </c:forEach>
					</select>
				</div>
				...
				<div class="custom-select selectRow">
					...
					<select id="searchoption4" name="searchoption4">
						<option value="" <c:if test="${agmtParam.searchoption4 == ''}">selected="selected"</c:if>>협약여부</option>
						<option value="0" <c:if test="${agmtParam.searchoption4 == '0'}">selected="selected"</c:if>>미협약</option>
						<option value="1" <c:if test="${agmtParam.searchoption4 == '1'}">selected="selected"</c:if>>협약완료</option>
					</select>
				</div>
				
				<!-- 공고명 검색--> 
				공고명 : <input type="text" name="searchword1" id="searchword1"  value="${agmtParam.searchword1}" style="" onkeypress="javascript:if(event.keyCode == 13) { fn_search(1); return false;}" />
				
				<!-- 신청과제명 검색 -->
				신청과제명 : <input type="text" name="searchword2" id="searchword2"  value="${agmtParam.searchword2}" style="" onkeypress="javascript:if(event.keyCode == 13) { fn_search(1); return false;}" />
				
				<a href="javascript:void(0);" id="btnSearch" class="btn btn-primary" onclick="fn_search(1)" >검색</a>
			 	
			 	<input type="hidden" name="pageSize" value="10"/>
				<input type="hidden"  id="page" name="page" value="${agmtParam.page}" />				
		...
		<div class="admin ma_t_20"> 
			<table class="table_h" cellpadding="0" cellspacing="0" border="0" id="admTable"> 
				<caption>
				 	 과제협약 -  번호, 소관기관 , 전담기관, 공고명, 신청과제명, 선정일, 협약여부
				</caption>
				<colgroup>
					<col style="width:5%">	
					<col style="width:11%">
