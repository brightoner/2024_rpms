...
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %>   
	...
	function fn_save(){
		...
		fn_showCustomConfirm("question","저장하시겠습니까?", function() {
				...
				var sendData = $("form[name='orgMngFrm']").serialize();
				
			    $.ajax({
			        url: '${ctxt}/opsmng/orgMng/updateOrgMng.do',
			        data: sendData,
			        type: 'POST', 
			        dataType: 'json',	 
	...
	function fn_delete(){
		...
		fn_showCustomConfirm("warning","삭제하시겠습니까 ?", function() {
			var form = document.orgMngFrm;
						form.action = '${ctxt}/opsmng/orgMng/deleteOrgMng.do';
						form.submit();	
...
<div id="right_content">   
	<h3 class="page_title" id="title_div"><span class="adminIcon"></span>기관 관리 상세</h3>  
		...
		<table class="table_v">  
			...
			<tbody>
				<tr>
					...
					<td>
						<div class="custom-select selectRow">
						 	<select name="org_gb" class="select" title="조회조건 선택">
								<option value="1" <c:if test="${data.org_gb == '1'}" >selected</c:if>>소관부처</option>
								<option value="2" <c:if test="${data.org_gb == '2'}" >selected</c:if>>전담기관</option>
								<option value="3" <c:if test="${data.org_gb == '3'}" >selected</c:if>>일반기관</option>
							</select>
						</div>
					</td>
					<th>기관분류</th>
						...
						<div class="custom-select selectRow">
						 	<select name="org_class" class="select" title="조회조건 선택">
									<c:forEach items="${org_class_code}" var="gbn">
											...
											<option value="${gbn.cd}" <c:if test="${data.org_class eq gbn.cd}">selected</c:if>>${gbn.cd_nm}</option>
				...
				<tr>
					<th>기관명</th>
					<td>
						<input type="text" name="org_nm" id="org_nm" maxlength="100" value="${data.org_nm}" >
					</td>
					<th>사업자 번호</th>  
					<td>				
						<input type="text" name="org_crm_no" id="org_crm_no" maxlength="40" value="${data.org_crm_no}" >
					</td>
				</tr>	
				<tr>
					<th>기관 주소</th>  
					<td colspan="3">				
						<input type="text" name="org_address" id="org_address" maxlength="100" value="${data.org_address}" >
					</td>     
				</tr>     			
				<tr>
					<th>실무자 명</th>  
					<td colspan="3">				
						<input type="text" name="org_charge_nm" id="org_charge_nm" maxlength="20" value="${data.org_charge_nm}" >
					</td>     
				</tr>
				<tr>
					<th>실무자 전화번호</th>  
					<td >				
						<input type="text" name="org_charge_phone" placeholder="'-'을 제외하고 숫자만 입력" id="org_charge_phone" maxlength="12"  value="${data.org_charge_phone}" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" >
					</td>  
					<th>실무자 휴대전화</th>  
					<td >				
						<input type="text" name="org_charge_mbtlnum"  placeholder="'-'을 제외하고 숫자만 입력" id="org_charge_mbtlnum" maxlength="12" value="${data.org_charge_mbtlnum}" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');">
					</td>    
				</tr>  
				<tr>
					<th>실무자 이메일</th>
					<td>									
						<input id="org_charge_email1" name="org_charge_email1" title="이메일아이디" placeholder="이메일 ID" type="text"  maxlength="30" autocomplete="off"  value = "${fn:split(data.org_charge_email,'@')[0]}" />
					</td>
						
					<td class="grid-input-0">
							<label for="org_charge_email2" class="blind">@</label>
							<input id="org_charge_email2" name="org_charge_email2" title="이메일주소" type="text"   maxlength="30" autocomplete="off" class="ma_l_3 txt"   value = "${fn:split(data.org_charge_email,'@')[1]}" />
