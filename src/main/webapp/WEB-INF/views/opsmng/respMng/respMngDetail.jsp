...
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %>   
	 ...
	 */
		...
		//현재 페이지 세팅
		fn_showCustomConfirm("warning","저장하시겠습니까?", function() {
		
			var sendData = $("form[name='respMngFrm']").serialize();
			
		    $.ajax({
		        url: '${ctxt}/opsmng/respMng/updateRespMng.do',
		        data: sendData,
		        type: 'POST', 
		        dataType: 'json',	 
	...
	function fn_delete(){
		...
		fn_showCustomConfirm("warning","삭제하시겠습니까 ?", function() {
				var form = document.respMngFrm;
			        		form.action = '${ctxt}/opsmng/respMng/deleteRespMng.do';
			        		form.submit();	
		});
...
<div id="right_content">   
	<h3 class="page_title" id="title_div"><span class="adminIcon"></span>연구자 관리 상세</h3>  
		...
		<table class="table_v">  
			...
			<tbody>
				<tr>
					...
					<td>
					 	<input type="text" name="resp_nm" id="resp_nm" maxlength="10" value = "${data.resp_nm}" >
					</td>
					<th>생년월일</th>
					<td>					 
				...
				<tr>
					<th>입사일</th>
					<td>					 
			 		 	<input type="date" name="resp_join_dt" id="resp_join_dt"   value = "${data.resp_join_dt}" max="9999-12-31" min="1111-01-01">
					</td>
					<th>퇴사일</th>
					<td>					 
			 		 	<input type="date" name="resp_quit_dt" id="resp_quit_dt"   value = "${data.resp_quit_dt}" max="9999-12-31" min="1111-01-01">
					</td>
				</tr>
				<tr>
					<th>성별</th>
					...
					<th>휴대전화</th>
					<td>
					 	<input type="text" name="resp_mbtlnum" id="resp_mbtlnum" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" value = "${data.resp_mbtlnum}" >
					</td>
				...
				<tr>
					<th>전화</th>
					<td>					 
		 		 		<input type="text" name="resp_phone" id="resp_phone" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" value = "${data.resp_phone}" >
					</td>
					<th>FAX</th>
					<td>
					 	<input type="text" name="resp_fax" id="resp_fax" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" value = "${data.resp_fax}" >
					</td>
				...
				<tr>
					...
					<td>
							...
							<input id="email1" name="email1" title="이메일아이디" placeholder="이메일 ID" type="text"  maxlength="30" autocomplete="off"  value = "${fn:split(data.resp_email,'@')[0]}" />
					</td>
						
					<td class="grid-input-0">
							<label for="email2" class="blind">@</label>
							<input id="email2" name="email2" title="이메일주소" type="text"   maxlength="30" autocomplete="off" class="ma_l_3 txt"   value = "${fn:split(data.resp_email,'@')[1]}" />
				...
				<tr>
					...
					<td>
						...
						<input type="text" name="resp_org" id="resp_org" maxlength="20"  value = "${data.resp_org}">
					</td>					
				</tr>
				<tr>
					<th>소속부서</th>
					<td>
						<input type="text" name="resp_dept" id="resp_dept" maxlength="200"  value = "${data.resp_dept}">
					</td>
					<th>직책</th>
					<td>
						<input type="text" name="resp_position" id="resp_position" maxlength="50"  value = "${data.resp_position}">
					</td>					
				</tr>   
				<tr>
					<th>학위구분</th>
					...
					<th>국가연구자번호</th>
					<td>					 
		 		 		<input type="text" name="resp_srch_no" id="resp_srch_no"  value = "${data.resp_srch_no}" >
					</td>		
				</tr> 
				<tr>
					<th>외부 전문가 여부</th>
				...
				<tr>
					<th>전공명</th>
					<td >
						<input type="text" name="resp_major_nm" id="resp_major_nm" maxlength="20" value = "${data.resp_major_nm}">
					</td>	
					<th>최종출신학교</th>
					<td >
						<input type="text" name="resp_school" id="resp_school" maxlength="40" value = "${data.resp_school}">
					</td>									
				</tr> 
				<tr>
					<th>전문분야</th>
					<td >
						<input type="hidden" name="resp_special1" id="resp_special1" maxlength="20" style="width: 70%" value = "${data.resp_special1}">
						<input type="text" name="resp_special1_nm" id="resp_special1_nm" maxlength="20" style="width: 70%" value = "${data.resp_special1_nm}" readonly="readonly">
						<a href="javascript:void(0);" onclick="javascript:fn_techSearch(1);" class="btn btn-secondary">  <i class="fas fa-search"></i></a>
					</td>
  
					
					<td >
						<input type="hidden" name="resp_special2" id="resp_special2" maxlength="20"  style="width: 70%"value = "${data.resp_special2}">
						<input type="text" name="resp_special2_nm" id="resp_special2_nm" maxlength="20"  style="width: 70%"value = "${data.resp_special2_nm}" readonly="readonly">
						<a href="javascript:void(0);" onclick="javascript:fn_techSearch(2);" class="btn btn-secondary"> <i class="fas fa-search"></i></a>
					</td>					
					<td >
						<input type="hidden" name="resp_special3" id="resp_special3" maxlength="20" style="width: 70%" value = "${data.resp_special3}">
						<input type="text" name="resp_special3_nm" id="resp_special3_nm" maxlength="20"  style="width: 70%"value = "${data.resp_special3_nm}" readonly="readonly"">
					...
					<th>비고</th>
					<td colspan="3">
						<textarea name="resp_etc" id="resp_etc" rows="5"  style="width: 100%;"  >${data.resp_etc}</textarea>
					</td>
