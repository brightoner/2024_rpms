...
function fn_respMngDtl(seq){
	...
	form.action = '${ctxt}/opsmng/respExpert/respExpertDetail.do';
	form.submit();	
}
...
<div id="container">
	<div id="divRefreshArea">
		<form name="writeForm" method="post" action="">
			<h3 class="page_title" id="title_div"><span class="adminIcon"></span>외부 연구자 DB</h3>  
			<div class="grid_box" style="grid-template-columns: 100px 150px 50px; "> 
				...
				<div class="custom-select selectRow">  
					...
					<select name="searchopt">					
						<option value="resp_nm" <c:if test="${respExpert.searchopt == 'resp_nm'}">selected="selected"</c:if>>연구자명</option>
						<option value="resp_org" <c:if test="${respExpert.searchopt == 'resp_org'}">selected="selected"</c:if>>소속기관</option>								
						<option value="resp_dept" <c:if test="${respExpert.searchopt == 'resp_dept'}">selected="selected"</c:if>>소속부서</option>								
						<option value="resp_position" <c:if test="${respExpert.searchopt == 'resp_position'}">selected="selected"</c:if>>직책</option>								
					</select>
				
				</div>
				<input type="text" name="searchword" id="searchword"  value="${respExpert.searchword}" style="" onkeypress="javascript:if(event.keyCode == 13) { fn_search(1); return false;}" />
				<a href="javascript:void(0);" id="btnSearch" class="btn btn-primary" onclick="fn_search(1)" >검색</a>
			 	<input type="hidden" name="pageSize" value="10"/>
				<input type="hidden"  id="page" name="page" value="${respExpert.page}" />
